...
router.delete("/delete_part", (req, res) => {
  ...
  try {
    const deletePartHasFitting = db.pool.query(
      "DELETE FROM `part_has_fitting` WHERE part_id = ? AND part_supplier_id = ?",
      [partId, sId]
    );
  } catch (err) {
    throw err;
  ...
  for (let i = 0; i < fittingIds.length; i++) {
    try {
      const deleteFitting = db.pool.query(
  ...
  //delete part
  try {
    const deletePart = db.pool.query(
